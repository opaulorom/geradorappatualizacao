...
const verticalSizes = {
  instagramStories: { width: 1080, height: 1920 },
    ...
    const handleDownloadSelected = useCallback(async () => {
      ...
      // Resolução fixa desejada
      const targetWidth = 1080; // Largura para o download
      const targetHeight = 1920; // Altura para o download
    
      setLoading(true); // Mostrar carregamento
    
      try {
        // Exportar como PNG forçando as dimensões desejadas
        const dataUrl = await toPng(node, {
          canvasWidth: targetWidth, // Largura final da imagem
          canvasHeight: targetHeight, // Altura final da imagem
          scale: targetWidth / node.offsetWidth, // Ajustar a escala proporcionalmente
        });
    
        // Criar link para download
        const link = document.createElement("a");
        link.href = dataUrl;
        link.download = `${selectedVerticalFormat || "imagem-gerada"}.png`; // Nome do arquivo
        link.click();
    
        setDownloadFeedback(
          `Imagem baixada com sucesso no formato ${selectedVerticalFormat || "padrão"} (1080x1920)!`
        ); // Feedback positivo
      } catch (error) {
        console.error("Erro ao baixar imagem:", error);
        setDownloadFeedback("Erro ao gerar a imagem. Tente novamente."); // Feedback negativo
      } finally {
